...
The repository now includes one MCP tool, `skepis_preflight`, served by `skepis-mcp` or `python -m skepis.mcp`. It loads the developer configuration and calls the existing `EvaluationGate.classify` path. It returns the scoped `CLEAN`, `EXPOSED`, and `UNKNOWN` partitions, memory and state availability, the configured policy for context, and monitoring coverage.

The tool is read-only. It does not read protected resources, write Sibyl state, journal a policy decision, run an evaluator, or expose a clean-claim result. Missing or mismatched Sibyl state remains `UNKNOWN` and `UNAVAILABLE`, and generic filesystem, Bash, browser, MCP, and Codex access remains `INCOMPLETE_MONITORING`.
...
Omit `task_ids` to classify every task registered in the configured benchmark. The MCP server exposes no other tools yet. The single-tool proof covers semantic task IDs, scope isolation, missing-store fail-closed behavior, and the absence of journal writes.
...
The current product surface is a local CLI plus one read-only MCP preflight tool. There is no frontend, hosted endpoint, or public demo URL.
...
| Read-only MCP preflight | Calls `EvaluationGate.classify` for the configured benchmark without journaling or running an evaluator | Agents can inspect scoped eligibility through the same core path without a second policy engine |
...
| MCP preflight reuses the classifier and remains read-only | `src/skepis/mcp.py:preflight` calls `EvaluationGate.classify`; `src/skepis/mcp.py:create_server` registers only `skepis_preflight` | `tests/test_mcp.py:McpPreflightTests.test_stdio_protocol_exposes_one_tool_and_preserves_scope_and_fail_closed_state` |
...
- The read-only MCP preflight proof exposed exactly one tool, classified a five-task semantic benchmark, preserved tenant scope isolation, returned `UNKNOWN` for a missing store, and emitted no COLD journal writes.
...
- The portable report is local and consumes a saved run result or the latest scoped Sibyl evaluation event. It does not add an MCP interface or claim generic agent telemetry.
